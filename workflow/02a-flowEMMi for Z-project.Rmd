...
```{r}
flowEMMi_mahalanobis <- function(data,gating_data){
  mu <- gating_data@mu
  ...
  for (col in 1:n_clusters-1){
    cutoff <- quantile(as.numeric(maha[,col]),0.95)
  ...
  maha_result <- list(maha_matrix=maha_data2,threshold=threshold,table_info=test)
  return(maha_result)
}
```
