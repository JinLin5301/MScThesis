...
================
Compiled at 2023-11-15 21:34:49 UTC
...
``` r
flowClust_roc <- function(data,gating_data,dimension,alpha){
  ...
  random_value <- data.frame()
  weight_pdf <- c()
  
  #compute pdf value of mix-t
  for (i in 1:num_points){
    for (j in 1:length(sigma)){
      pdf <- mvtnorm::dmvt(random_data[i,],delta=mu[,j],sigma=sigma[[j]],df=4,log=F)
      weight_pdf[j] <- pdf*w[j]
      }
    random_value[i,1] <- sum(weight_pdf)
    }
  
  adjust_value <- random_value*(max(random_boxcox)-min(random_boxcox))^dimension
  roc.points <- data.frame(cell=NA,volume=NA)
  ...
  for (alpha in c(0.1,0.9)){
    ...
    # volume
    inside_distribution <- c()
    for (i in 1:num_points){
      if(adjust_value[i,1]>threshold){inside_distribution[i]<-0}
      else {inside_distribution[i]<-1}
    }
    sum1 <- trunc(sum(inside_distribution)/num_points*ncells)
    sum2 <- sum(inside_distribution)/num_points*(65535-200)^dimension
...
for (i in 1:5){
  ...
  DAPI_fC_flex_roc[[name]] <- flowClust_roc(data0,gating_data,2,alpha)
